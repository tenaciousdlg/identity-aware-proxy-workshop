...
const SERVICE_NAME = process.env.SERVICE_NAME || 'bob-app';
...
// Main endpoint (should only be accessible to admin users via Envoy RBAC)
app.get('/', (req, res) => {
  ...
  const response = {
    service: SERVICE_NAME,
    message: `Welcome to Bob's personal service!`,
    description: 'This service is only accessible by Bob. Envoy enforces that only users named "bob" can access this endpoint.',
    warning: '🔒 This is Bob\'s private workspace',
    ...
    security_note: 'If you can see this, Envoy has validated that you are Bob.'
  };
